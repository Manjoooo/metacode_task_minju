...
2. impression, click, conversion 이벤트를 분리하여 수집하는 문제
...
| 운영 가시성                 | Iceberg snapshots/files/history metadata table 기반 health query 작성                                  |
...
Criteo 원본의 `timestamp`와 `conversion_timestamp`는 실제 날짜가 아니라 상대 초 단위 시각이다. 그대로 timestamp로 변환하면 1970년대 파티션이 생길 수 있으므로, Producer는 기준 시각을 정하고 상대 초를 더해 논리적 event_time을 생성한다.
...
```mermaid
flowchart LR
    A["Criteo TSV<br/>pcb_dataset_final.tsv"] --> B["Event Replay Producer<br/>row → event 변환<br/>event_time 기준 replay"]
    B --> C1["ad-impressions"]
    B --> C2["ad-clicks"]
    B --> C3["ad-conversions"]
    C1 --> D["Spark Structured Streaming<br/>3 topics consume<br/>checkpoint<br/>Kafka metadata 저장"]
    C2 --> D
    C3 --> D
    D --> E["Bronze Raw Parquet<br/>append-only<br/>event_type/raw_date/raw_hour"]
    E --> F["Silver Iceberg<br/>processed_ad_journeys<br/>dedup + journey 구성<br/>conversion_delay 계산"]
    F --> G["Gold Iceberg<br/>campaign_summary<br/>hourly incremental MERGE<br/>daily 30d rebuild"]
    G --> H["Streamlit Dashboard<br/>Business KPI<br/>Operation Metrics"]
    I["Operation Jobs<br/>Health Check<br/>Compaction<br/>Backfill"] -.-> F
    I -.-> G
    I -.-> H
```
...
실시간처럼 보이기 위해 모든 계층을 하루 1회 batch로 처리하지 않는다. 수집은 streaming으로 유지하고, Silver/Gold는 주기적 incremental batch로 갱신하며, late conversion 보정은 하루 1회 최근 30일 window로 수행한다.

| Layer            | Job                           | 처리 방식                      |             주기 | 실행 시각  | 목적                          |
| ---------------- | ----------------------------- | -------------------------- | -------------: | ------ | --------------------------- |
| Producer         | `criteo_event_replay`         | Event replay               |   테스트 실행 또는 상시 | 수동/상시  | 오프라인 TSV를 실시간처럼 발행          |
| Bronze           | `kafka_to_raw`                | Spark Structured Streaming | 1분 micro-batch | 상시     | Kafka 이벤트 원본 보존             |
| Silver           | `raw_to_processed`            | Incremental Batch + MERGE  |            1시간 | 매시 10분 | dedup, journey 구성, delay 계산 |
| Gold Incremental | `processed_to_summary_hourly` | Incremental Batch + MERGE  |            1시간 | 매시 20분 | 최신 dashboard KPI 반영         |
| Gold Late Window | `summary_rebuild_30d`         | 30일 window 재집계 + MERGE     |             1일 | 01:30  | late conversion 보정          |
| Compaction       | `compact_iceberg`             | Maintenance                |             1일 | 03:00  | small file 정리               |
| Health Check     | `health_queries`              | SQL check                  |             1일 | 09:00  | 운영 헬스체크                     |

### 8-1. 왜 Gold를 두 단계로 나누는가

Gold를 하루 1번만 갱신하면 dashboard freshness가 낮아진다. 반대로 모든 conversion이 들어올 때마다 Gold MERGE를 실행하면 snapshot과 small file이 과도하게 증가할 수 있다.

따라서 본 프로젝트는 다음 두 가지 Gold job을 분리한다.

1. `processed_to_summary_hourly`

   * 최근 1~2시간 또는 당일 데이터만 반영
   * dashboard 최신성 확보
   * 매시 20분 실행

2. `summary_rebuild_30d`

   * 최근 30일 impression_date window 전체를 재집계
   * late conversion 보정
   * 매일 01:30 실행

이 구조는 실시간 수집과 운영 가능한 batch 집계 사이의 균형을 맞춘다.
...
3. D일 summary는 D+1 01:30 late window job에서 최종 보정된다.
4. 학습용 feature를 만들 경우 cutoff time 이후 ingest된 conversion은 사용하지 않는다.
5. batch job은 반드시 `--run-date-start`, `--run-date-end`를 명시적으로 받아 처리 기간을 고정한다.
...
| Gold            | summary 갱신 시간                             | hourly job과 30일 rebuild 분리          |
...
4. Gold hourly job은 최신 KPI를 빠르게 반영한다.
5. Gold late window job은 최근 30일 impression_date window를 재집계한다.
6. Iceberg MERGE INTO로 기존 campaign_summary row를 update한다.
...
2. Gold hourly MERGE는 매시 20분, late window 재집계는 01:30, compaction은 03:00에 실행하여 시간대를 분리한다.
...
| 규모            | 실행 방식                                               |
| ------------- | --------------------------------------------------- |
| 하루치           | `--run-date-start`, `--run-date-end` 지정 후 수동 실행     |
| 1주일 내외        | 별도 backfill script에서 기간 지정                          |
...
| 2  | daily/hourly row count     | 일자·시간별 데이터 누락/급증 확인                          |
...
| 9  | Kafka topic distribution   | event_type별 수집량 확인                           |
| 10 | pipeline freshness         | event_time, producer_time, ingest_time 차이 확인 |
...
* daily/hourly processed row count
* duplicate event count
...
streaming ingestion과 hourly MERGE job과 충돌하지 않도록 D-1 이전 partition을 대상으로 한다.
...
| `code/pipelines/processed_to_summary_hourly.py` | 최근 1~2시간 또는 당일 window 기준 Gold incremental MERGE                                  |
| `code/pipelines/summary_rebuild_30d.py`         | 최근 30일 impression_date window 재집계 및 Gold MERGE                                   |
...
| Gold Hourly      | dashboard freshness와 MERGE 비용 충돌 | hourly window 축소, changed-date만 MERGE           |
| Gold 30d Rebuild | 최근 30일 재집계 비용 증가                 | changed impression_date 기반 재집계                  |
...
4단계: Gold Incremental
- 최근 1~2시간 또는 당일 데이터 기준 campaign summary 갱신
- 매시 20분 MERGE 실행
- dashboard freshness 확보

5단계: Gold Late Window
- 최근 30일 impression_date 기준 재집계
- late conversion 반영
- 매일 01:30 MERGE 실행
...
| Gold Hourly      | hourly summary query        | 최신 KPI가 1시간 단위로 반영되는지 확인                                            |
| Gold 30d Rebuild | late window query           | 최근 30일 late conversion이 과거 summary에 반영되는지 확인                        |
...
| Gold daily batch만 가정                 | Gold hourly incremental + daily 30d rebuild로 분리    |
...
└── screenshots/
    ...
    ├── 05_gold_summary_hourly.png
    ├── 06_gold_late_window_merge.png
    └── 07_dashboard.png
```
...
실시간성을 위해 Bronze는 Spark Structured Streaming으로 상시 수집하고, Silver와 Gold incremental summary는 1시간 단위로 갱신한다. 동시에 데이터 분석 결과 conversion delay가 최대 약 30일까지 발생하므로, late conversion 반영을 위해 매일 01:30에 최근 30일 window를 재집계한다.

또한 Iceberg metadata table을 이용해 snapshot freshness, duplicate count, late conversion count, file count, average file size를 모니터링하고, compaction을 자동화하여 small file 문제를 관리한다.
...
5. Gold Iceberg에서 hourly incremental MERGE와 daily 30d late window rebuild 분리
